package com.iquest.advancedframeworks.internetbanking.persistence.dao;

import com.iquest.advancedframeworks.internetbanking.persistence.model.DepositTransaction;

/**
 * The DepositTransactionDao interface adds operations which can be performed with DepositTransaction entities besides
 * the CRUD operations inherited from the GenericDao interface.
 * 
 * @author Nicoleta Barbulescu
 *
 */
public interface DepositTransactionDao extends GenericDao<DepositTransaction> {

}
